...
TRACKMENOT.TMNSearch = function () {
	...
	var typeoffeeds = [];
	var zeitgeist = ["physorg", "quantum", "dogs", "chemistry", "phonon", "PC", "Files", "language", "jungle", "antique", "roman", "bargain", "pottery", "crabs", "whales", "sea", "pi", "series", "maths", "Blackcurrents", "flies", "insects", "calm", "butterflies", "room", "parks", "bikes", "Kites", "steam", "best games", "lottery", "nobel", "smile", "beatles", "cobwebs", "kindly", "maxwell", "niels bohr", "statues", "crime novels", "comics", "brothers", "research", "deserves", "vegan", "moon", "sun", "jam jars", "pancake", "mercury", "electron", "ok", "Hertz", "botany", "fossil", "particle", "virus", "bacteria", "phase", "birds", "hospital", "smells", "noses", "gravity", "weak force", "strong force", "dark", "magnet", "mineral", "moorland", "floods", "corruption", "scandal", "masons", "observatory", "petri", "geology", "granite", "space", "hyper", "elements", "protons", "cancer", "eyes", "look", "watches", "wrist", "farm", "garden", "reptiles", "spiders", "metal", "flags", "scammers", "celt investigate", "stone circles", "castles", "happy songs", "good question", "good answer", "halls"];
	var tmn_timeout = 14058;
	var prev_engine = "None";
	var burstEngine = '';
	var burstTimeout = 8603;
	...
	function sendLogToOption() {
		worker_opt.port.emit("TMNSendLogs", {
			logs : tmnLogs
		});
	}
	...
	function sendQueriesToOption() {
		...
		worker_opt.port.emit("TMNSendQueries", {
			queries : allqueries,
			sources : TMNQueries
		});
	}
		...
		try {
			...
			if (selected_engine) {
				var sel_engine_name = selected_engine.name.toLowerCase()
					if (init_engines.filter(function (a) {
							return a.id == sel_engine_name
						}).length == 1) {
	...
	function validateFeeds(param) {
		TMNQueries.rss = [];
		var added_feed = param.feeds.split('|').filter(function (x) {
				return (feedList.split('|').indexOf(x) < 0);
			});
	...
	function extractQueries(html) {
		...
		for (var i = 0; i < l.length; i++) {
			...
			if (/([a-z]+ [a-z]+)/i.test(l[i])) {
				...
				var matches =  reg.exec(l[i]);//l[i].split(" ");
	...
	function addQuery(term, queryList) {
		...
		if (containsObj(queryList,term))
			return false;
		debug("Adding terms:"  + term)
		queryList.push(term);
	...
	function addRssTitles(xmlData, feedUrl, showOptions = false) {
		...
		if (showOptions) {
			worker_opt.port.emit("TMNShowValidatedFeed", {
				name : feedObject.name,
				words : feedObject.words
			});
		}
    ...
    function scheduleNextSearch(delay) {
        ...
        if (delay > 0) {
            if (!isBursting()) { // randomize to approach target frequency
                delay = parseInt(delay * 0.75) + (delay * (Math.random() / 2));
        ...
        if (isBursting())
        {
        ...
        else
           {
            ...
            tmn_errTimeout = timer.setTimeout(rescheduleOnError, delay + (delay * (Math.random() * 3))); // tmn_errTimeout = timer.setTimeout(rescheduleOnError, delay * 3);
            delay = parseInt(delay * 0.66) + (delay * (Math.random() / 3));
	...
	function sendClickEvent() {
		try {
			worker_tab.port.emit("TMNClickResult", {
	...
	return {
		...
		startTMN : function () {
			...
			searched_engines = engines.filter(function (a) {
					return searched_engines_ids.indexOf(a.id) >= 0
				})
